...
(function () {
  ...
  /* Hero background: if images/hero.jpg hasn't been added yet, hide the
     broken image and let the warm gradient on .hero__media show through. */
  function attachHeroFallback() {
    if (!heroImage) return;
    heroImage.addEventListener("error", function handler() {
      heroImage.removeEventListener("error", handler);
  ...
  function buildCard(product) {
    ...
    // Store brand is the primary identity, marked directly on the photo.
    var storeTag = document.createElement("span");
    storeTag.className = "product-card__mark";
