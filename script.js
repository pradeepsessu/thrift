/* =====================================================
   SCRIPT.JS
   =====================================================
   Everything here reads from the `products` array in
   products.js. You should never need to edit this file
   to add a product — only products.js.
   ===================================================== */
...
(function () {
  ...
  var availableCheckbox = document.getElementById("filterAvailable");
  ...
  /* ---------------------------------------------------
     IMAGE PLACEHOLDER (used if a product image is
     missing, e.g. before real photos are added)
     --------------------------------------------------- */
  function placeholderFor(label) {
    var safeLabel = (label || "Image").slice(0, 24);
    var svg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="600" height="800">' +
      '<rect width="100%" height="100%" fill="#E7EAE2"/>' +
      '<text x="50%" y="50%" font-family="monospace" font-size="24" ' +
      'fill="#6B6862" text-anchor="middle" dominant-baseline="middle">' +
      safeLabel + '</text></svg>';
  ...
  /* ---------------------------------------------------
     ...
     so new brands/waists/sizes show up with no extra edits)
     --------------------------------------------------- */
  ...
  function getFilteredProducts() {
    ...
    var filtered = products.filter(function (p) {
      if (state.availableOnly && p.sold) return false;
  ...
  function buildCard(product) {
    ...
    tag.textContent = product.waist + "×" + product.length + " · " + product.size;
    ...
    body.innerHTML =
      ...
      '<span class="product-card__meta">' +
        ...
        '<span class="product-card__specs">' + product.waist + '&times;' + product.length + '</span>' +
      '</span>';
  ...
  function openModal(productId) {
    ...
    var message = encodeURIComponent("Hi! I'm interested in the " + product.name + " (" + product.brand + ").");
  ...
  availableCheckbox.addEventListener("change", function () {
    state.availableOnly = availableCheckbox.checked;
    renderGrid();
  });
  resetButton.addEventListener("click", function () {
    state = { search: "", brand: "", waist: "", size: "", sort: "newest", availableOnly: false };
    searchInput.value = "";
    brandSelect.value = "";
    waistSelect.value = "";
    sizeSelect.value = "";
  ...
  function applyShopSettings() {
    ...
    igLink.href = "https://ig.me/m/" + SHOP_SETTINGS.instagramUsername;
